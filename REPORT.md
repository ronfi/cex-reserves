...
| HTX | 11/11 | 8,292 | 8,213 | <span class="ok">+1.0%</span> | 19,933 / 20,472(08-01) | <mark class="r">-59.5%</mark>(自报钱包含 BTC-TRC20 10,399 + 托管 1,689 + BTC-SOL/jWBTC 175;原生 8,209 vs 直读 +2%) |
...
- 新增的"直读原生 ETH"只计以太坊主网地址上的原生 ETH(不含 stETH 等凭证与 L2),脚本 `tools/eth_native_units.py`,09-05 补读;自报 ETH 钱包是**全链口径**(含 L2、质押凭证与托管),所以"直读原生 − 自报钱包"普遍为负,只作参照,不按 §3.1 的红标规则判。例:HTX 自报钱包 122,627 中托管 91,525、stETH 29,158,主网原生 ETH 仅 1,504(其 GitHub 快照 CSV),与直读 11,118 在同一量级;Binance −18% 与 OKX −42% 主要是页面含多链与地址集不同。
...
| 3 | **BTC 储备 51% 是 Poloniex 发行的 BTC-TRC20,公布地址与 PoR 页面上未发现对应抵押物(链上检索不能证否)** | 自报 BTC 20,472 = 原生 8,209 + BTC-TRC20 10,399 + 托管 1,689 + 其他 175;原生链上现值 8,073 = 用户负债 19,933 的 **41%** | `--chain btc,tron --ex htx`;BTC-TRC20 见 §7.2 |
...
| 5 | **TRX 占储备 47%,69% 质押中,而全市场盘口装不下它的 1%** | 18 址持 9.77B TRX = 供给 10.3%(可用 3.04B + 质押中 6.73B,质押占 69%);用户 TRX 负债 8.85B ⇒ 即时可付 34%;十所现货 ±2% 深度合计 $17.6M,HTX 持仓 $3.2B | trongrid `getaccount`;各所深度接口 |
| 6 | **平台币 HTX 计入储备 5.7%,而 HTX 自持其供给约 23%,仅本所有成交** | Tron 主链 totalSupply **999.99 万亿枚**(2026-09-07 合约直读);⚠ HTX 自持量本周未复核 —— 上期记"18 址持 2.31 万亿"与同句"自持 23%"不自洽(2.31/1000 = 0.23%),持币地址不在本报告的 TRX 冷钱包清单内,已列入下期核实;Binance/OKX 未上市,五家挂牌所日成交 $0–5 万;九所无永续 | Tron JSON-RPC;各所行情接口 |
...
| HTX | TRX | <mark class="r">47%</mark>(自报口径) | $3.27B(本方 18 址直读 97.7 亿枚 × $0.335;<mark class="r">69% 质押中</mark>) | $72.4M(10 所;HTX 自家 30 日均 $2.6M) | $394.9M | $16.9M(十所) | ≈45 天 |
| HTX | HTX 币 | 5.7%(自报口径) | $0.37B ⚠ 本周未复核 | $28.2M(6 所;仅 HTX 有成交) | $31.5M | $0.42M(六所) | ≈13 天 |
