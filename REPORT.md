...
> 各表标题另有各自的读取时间。全部数字可用本仓库 `tools/cex_reserves_verify.py` 重新读取;输出快照在 `data/cex_reserves_2026-09-14.json`。
> 口径纪律:**链上直读 > 官方 PoR 页面 > 公开聚合器 > 媒体**。前三类进表;媒体只作线索,标 ⚠,不进表。
> 本页不对任何交易所的偿付能力作判断,只呈现可核验的事实,以及各来源之间的差异。**非投资建议。**

## 0. 摘要

1. **关联币占储备的分布:20 家里 18 家低于 20%,只有两家超过三成 —— 最高 78%,其次 32%。** 8 家完全没有关联币;**有关联币的 12 家,中位数 13%**(⚠ 口径:若把 8 家零值一并计入,20 家的中位数是 2.5% —— 两个中位数不是一回事,引用时须说明是哪一个)。逐所见 §4.2。
2. 头部交易所里,**同时具备四类关联资产的只有 HTX**:平台币(HTX 币)、关联稳定币(USDD)、自发包装币(BTC-TRC20、HBTC)、JustLend 生息凭证(stUSDT/jUSDD)。其余各所最多一种(分类与 FTX 三条判据见 §4)。
3. **HTX 自报储备率"均超 100%",但按链上可核口径:USDT 自有钱包只覆盖负债的 4.8%,BTC 42%,ETH 25%;总储备的 18.1% 在不披露身份的"第三方托管",47.5% 是 TRX 且其中 69% 处于质押。**(自报数为 2026-09-01 页面快照,链上数为 09-14 直读)
4. Binance / OKX 的储备在 ETH 链上可 95–99% 对上;Bitfinex、Gate、KuCoin、Gemini 在 98–101%;BTC 链十一所全部对上(差 ≤1.0%)。各所均自报储备率 >100%;本报告只能核验其中可在链上读到的部分,各所可核比例见 §3。
...
## 2. 全景:链上储备前 20 名(DefiLlama 公开地址口径,读取于 2026-09-14 01:08 UTC)
...
- ⚠ Bitstamp 行为聚合器 09-04 22:30 UTC 读数;本报告直读 34,108 BTC(§3.1)。
...
| 5 | Robinhood(经纪商) | $14.7B | 77% | 21% | 0% | 0% | -0.0% |
...
| 11 | Bitstamp | $4.7B | 68% | 21% | 1% | 0% | +129.4% ⚠ 非资金流入(见读法) |
...
| 16 | Poloniex | $1.6B | 49% | 40% | 1% | 1%(BTT,JST,NFT,SUN,TRX,USDD,WIN) | +45.4% ⚠ 非客户流入(见读法与 §7);窗口仅 191 天(起 02-28) |
| 17 | Bitkub | $1.5B | 65% | 14% | 3% | 0% | +34.8% |
| 18 | SwissBorg | $1.0B | 43% | 14% | 6% | 15%(BORG) | -0.8% |
| 19 | BitMEX | $0.8B | 87% | 0% | 13% | 0% | -60.4%(2026-09-23 自愿关停,清退前提币) |
| 20 | OSL | $0.8B | 73% | 19% | 6% | 0% | +7.9% ⚠ 窗口仅 235 天(起 01-15) |

**读法**

- 关联币占储备超过三成的只有两家:**HTX 78%、Bitfinex 32%**。LEO 是 iFinex 2019 年发行的平台币(非稳定币,靠营收回购销毁支撑),与 BNB、OKB、GT、KCS 同类,按同一规则标注。
...
§2 是聚合器口径;本节是**本报告自己读链**:拿各所公布的地址(官方 PoR 清单或DefiLlama-Adapters源码,见 `tools/cex_addresses.json`),逐地址读 BTC / ETH 全资产 / Tron,再与聚合器对账。**表由 `tools/make_tables.py` 从 `data/cex_reserves_2026-09-14.json` 生成**,每个地址的读数与失败记录都在该文件里。

### 3.1 BTC 链(mempool.space 直读 vs 聚合器;差值 ≤1% 绿、>5% 红)

| 所 | 公布地址(读到/总) | 直读 BTC | 聚合器 BTC(≈) | 差 | PoR 自报 BTC:用户 / 钱包(快照日) | 直读 − 自报钱包 |
|---|---|---|---|---|---|---|
| Binance | 61/61 | 642,725 | 642,725 | <span class="ok">+0.0%</span> | 656,644 / 658,293(08-01) | -2.4% |
| Bitfinex | 3/3 | 150,156 | 150,135 | <span class="ok">+0.0%</span> | — | — |
| Bybit | 25/25 | 57,910 | 57,845 | <span class="ok">+0.1%</span> | 56,438 / 59,064(07-23) | -2.0% |
| Gate | 13/13 | 18,936 | 18,905 | <span class="ok">+0.2%</span> | 22,436 / 27,550(08-19) | <mark class="r">-31.3%</mark>(DefiLlama-Adapters清单 13 址不是页面全部钱包(Gate 未公布 BTC 地址)) |
| Bitget | 19/19 | 30,021 | 30,012 | <span class="ok">+0.0%</span> | 26,811 / 35,837(08-20) | <mark class="r">-16.2%</mark>(适配器 19 址不是页面全部钱包;页面钱包含 BSC/Lightning 等链 535 枚) |
| MEXC | 34/34 | 11,754 | 11,752 | <span class="ok">+0.0%</span> | 4,107 / 12,202(09-09) | -3.7% |
| Gemini | 4/4 | 56,251 | 56,242 | <span class="ok">+0.0%</span> | — | — |
| Deribit | 17/17 | 50,442 | 50,434 | <span class="ok">+0.0%</span> | — | — |
| HTX | 11/11 | 8,203 | 8,102 | +1.3% | 19,498 / 20,252(09-01) | <mark class="r">-59.5%</mark>(自报钱包 20,252 = 交易所钱包 18,563 + 托管 1,689;交易所钱包含 BTC-TRC20,09-14 链上 10,334 枚,扣除后约 8,229,与本表直读 8,203 差 −0.3%) |
| Crypto.com | 8/8 | 22,492 | 22,725 | -1.0% | — | — |
| Bitstamp | 507/507 | 34,108 | 40,043 | <mark class="r">-14.8%</mark> | — | — |

**读法**

- **十一所全部对上**(差 ≤1.0%),聚合器的 BTC 数可直接引用。
- HTX 的行是DefiLlama-Adapters清单 11 址;其官方 PoR 清单为 7 址,链上 7,884 枚(§6.2)。
...
| 所 | 地址数 | 全资产直读 | 聚合器口径直读 | 聚合器 | 覆盖 | 失败地址 | 直读原生 ETH | PoR 自报 ETH:用户 / 钱包(快照日) | 直读原生 − 自报钱包 |
|---|---|---|---|---|---|---|---|---|---|
| Binance | 37 + 锁仓 2 | $68.58B | $68.58B | $68.88B | <span class="ok">100%</span> | 0 | 3,118,554 | ? / 3,991,221(08-01) | -22% |
| OKX | 323 | $13.19B | $13.19B | $13.81B | <span class="ok">96%</span> | 0 | 1,024,335 | 1,725,703 / 1,749,426(2026-08) | -41% |
| Bitfinex | 9 | $7.12B | $7.12B | $7.03B | <span class="ok">101%</span> | 0 | 281,598 | — | — |
| Gate | 91 | $3.48B | $3.48B | $3.17B | 110% | 0 | 170,162 | 375,430 / 458,203(08-19) | -63% |
| Bitget | 80 | $1.42B | $1.42B | $1.45B | <span class="ok">98%</span> | 0 | 112,120 | 123,688 / 190,090(08-20) | -41% |
| Gemini | 5 | $0.81B | $0.81B | $0.82B | <span class="ok">99%</span> | 0 | 287,305 | — | — |
| HTX | 57 | $0.22B | $0.14B | $0.13B | 108% | 0 | 11,118 | 114,984 / 116,757(09-01) | -90% |
| KuCoin | 96 | $1.58B | $1.58B | $1.59B | <span class="ok">100%</span> | 0 | 90,156 | 101,664 / 118,497(08-31) | -24% |
| Bitstamp | 64 | $0.58B | $0.58B | $1.15B | <mark class="n">50%*</mark> | 0 | 167,082 | — | — |
...
- Binance 行含锚定代币锁仓 2 址($12.3B),不是用户资产。"锚定代币锁仓"指 Binance 为其在 BNB Chain 上发行的 Binance-Peg 代币(BSC 版 USDT、USDC、ETH 等)在以太坊主网锁定的原生抵押品,负债方是这些锚定币的持有人,而非交易所客户;地址来自 Binance 的 lockinfo 端点,不在其 PoR 清单内;已向聚合器提出拆分提案(DefiLlama-Adapters PR #20885,维护者已关闭未合并)。
- "失败地址"全部为 0(OKX 一址补读后清零)。
- 末列为各所官方 PoR 自报的 ETH(用户 / 钱包,快照日各异),口径是**全链 ETH**(含 L2、质押凭证与托管),与本表以太坊主网美元读数不是同一口径,只作参照,不算差值。
- 新增的"直读原生 ETH"只计以太坊主网地址上的原生 ETH(不含 stETH 等凭证与 L2),脚本 `tools/eth_native_units.py`,09-05 补读;自报 ETH 钱包是**全链口径**(含 L2、质押凭证与托管),所以"直读原生 − 自报钱包"普遍为负,只作参照,不按 §3.1 的红标规则判。例:HTX 09-01 自报钱包 116,757 中托管 87,543、交易所钱包 29,213(页面只分这两栏;分链构成只在 GitHub 快照 CSV 上,该 CSV 仍停在 08-01,见 §6.2 第 8 条),与直读 11,118 在同一量级;Binance −18% 与 OKX −42% 主要是页面含多链与地址集不同。

### 3.3 Tron 链(trongrid `getaccount` 含四项质押 + USDT-TRC20 直读)

| 所 | 地址数 | TRX 可用 | TRX 质押中 | USDT-TRC20 | PoR 自报 TRX:用户 / 钱包 | 直读 TRX − 自报钱包 | PoR 自报 USDT(全链):用户 / 钱包 |
|---|---|---|---|---|---|---|---|
| Binance | 25 | 2,273M | 0M | 1,682.8M | — | — | — |
| OKX | 23 | 145M | 527M | 271.9M | — | — | 8,118M / 8,637M(2026-08) |
| Bitfinex | 2 | 28M | 42M | 74.1M | — | — | — |
| Gate | 11 | 13M | 84M | 79.9M | 58M / 179M(08-19) | <mark class="r">-45.5%</mark>(DefiLlama-Adapters 清单 11 址不是页面全部钱包(Gate 未公布 TRX 地址)) | 660M / 721M(08-19) |
| Bitget | 29 | 4M | 0M | 231.6M | — | — | 1,454M / 1,456M(08-20) |
| HTX | 18 | 3,015M | 6,733M | 0.0M | 8,620M / 9,384M(09-01) | +3.9% | 872M / 588M(09-01) |
| KuCoin | 24 | 18M | 58M | 102.2M | — | — | 955M / 1,059M(08-31) |

**读法**

- **聚合器不可引用。** 它漏计 USDT-TRC20(Bitget 公布地址持 247M,聚合器记 0),其 `eth_getBalance` 读法也不含质押中的 TRX。
- Tron 的"冻结"(freeze)是其官方术语,指把 TRX 质押给网络换取带宽、能量与投票权:所有权不变、可随时发起解押、解押后 14 天到账;它不是借贷抵押,也不是被平台或司法冻结。本报告统一写"质押"。质押有四个去处:V1 质押、V2 自持质押、**委托给他址的质押**(TRX 仍归本址)、解押队列;表中"质押中"四项全计。OKX 的 5.17 亿、Poloniex 的 2,400 万都是委托质押,只读 `balance + frozenV2` 会看不到(§7.1)。
- HTX 表中 18 个 Tron 地址是DefiLlama-Adapters清单里的 TRX 冷钱包,上面没有 USDT 属正常;**HTX 官方 PoR 的 USDT-TRC20 另有 5 个地址**:08-01 快照 13.24M,09-14 链上 <mark class="r">1.91M</mark>(低于用户 USDT 负债的 1%,§10 规则;`TK86…` 的 11.33M 已转空);USDT-ERC20 1 址 1.05M → 0。用户 USDT 负债 926M 的其余部分在"ThirdParty"(§6.2)。
- 末两列为各所官方 PoR 自报的 TRX 与 USDT(用户 / 钱包);USDT 是**全链合计**(ERC20 + TRC20 + 其他链),本表 USDT-TRC20 只是其中一条链,故不算差值。HTX 的 TRX 自报钱包 9,384M(09-01)与本表 18 址 9,747M(可用 + 质押,09-14)同量级;其 USDT 自报钱包 588M 中 547M 在"ThirdParty"(93%,§6.2)。
- "直读 TRX − 自报钱包":直读 TRX(可用 + 质押)与自报 TRX 钱包同链同口径,可直接比,按 §3.1 规则超过 5% 标红并注明原因。Gate −92% 是地址集差(DefiLlama-Adapters 清单 11 址,Gate 未公布 TRX 地址);HTX +3.4%。USDT 不算差值,因自报为全链合计。
...
| 所 | 关联币 | 占储备 | 硬资产 |
|---|---|---|---|
| HTX | TRX + HTX 币 | <mark class="r">**78%**</mark> | 18% |
...
| OKX | OKB | 5% | 86% |
| Bybit | MNT | 4% | 74% |
| Poloniex | 关联体系代币 | 1% | 90% |
| Crypto.com | CRO | 1% | 90% |
| Robinhood、Gemini、Deribit、Bitstamp、HashKey、Bitkub、BitMEX、OSL | — | 0% | 79–100% |

- 超过三成的两家:HTX 78%、Bitfinex 32%。两家的差别在硬资产:Bitfinex 硬资产 68%,是 LEO 的两倍;HTX 硬资产 18%,关联币是它的四倍以上。关联币是否超过硬资产,是 §4.4 判据 ① 的口径。
- 上表是聚合器口径(只认交易所公布的地址,且剔除自发/关联资产)。按 HTX 官方 PoR 快照的自报口径(2026-09-01;§5、§6.2):TRX 占 47.5%、HTX 币 6.1%,另 BTC 行的 51% 是 Poloniex 发行的 BTC-TRC20(§7)。
...
| 规模与持有(一手) | BTCTRON 供给 17,545 枚,HTX 持 10,334(2026-09-14 直读),占其 PoR BTC 行 51%;HBTC 供给仅 969.49 枚,HTX 地址持 ≈540(56%) |
...
| 规模与持有(一手) | HTX 自有钱包 USDT 中 73% 是 stUSDT(39.55M;⚠ 该构成来自 08-01 的分链快照,09-01 的分链数据 HTX 尚未发布,§6.2);Poloniex 快照 sUSDS 912.6M、stETH 247.8k(§7);Binance WBETH $8.4B;aEthUSDT 池 09-03 链上:总供给 ≈$29.5 亿、池内现金 $2.36 亿、利用率 92%(<https://etherscan.io/token/0x23878914EFE38d27C4D67Ab83ed1b93A74D4086a>) |
...
| 所 | ① 关联币是储备主体 | ② 持仓 ÷ 主流所 30 日均成交(全量出清需天数,§8) | ③ 负债端(§5) |
|---|---|---|---|
| HTX | <mark class="r">**是**(78% vs 硬资产 18%)</mark> | TRX ≈45 天、HTX 币 ≈13 天 | 公布逐币负债;**19% 储备在托管方未在页面披露,USDT 自有钱包覆盖负债 5.8%**(§6.2) |
| Bitfinex | 否(32% vs 68%) | LEO ≈26,700 天 | 无 PoR 页,负债未公布 |
| KuCoin | 否(15% vs 59%) | KCS ≈96 天 | 仅公布比率 110% |
| SwissBorg | 否(15% vs 63%) | 未测 | 无 PoR 页 |
| Binance | 否(16% vs 72%) | BNB ≈177 天 | 公布逐币负债,仅自有钱包 ÷ 负债 100.9% |
| Gate | 否(15% vs 53%) | GT ≈1,430 天 | 仅公布比率 127% |
| MEXC | 否(11% vs 68%) | 未测 | 仅公布比率 141% |
| OKX | 否(5% vs 86%) | 未测 | 公布逐币负债,103.0% |

**读法**

- ① 只对一家成立。
- ② 单看不区分谁更危险:出清天数从十几天到几万天,要与 ① 并读;关联币占比越高、成交越稀薄,按可成交量折算后的储备缩水越多。
- ③ 公布逐币负债的只有 Binance、OKX、HTX 三家;其余要么只给一个比率,要么没有 PoR 页。
- 三条同时成立的所,前 20 名里没有;最接近的是 HTX:① 成立,② 出清约需 45 天(TRX 是这些关联币里成交最活跃的),③ 公布了负债,但五分之一储备在托管方未在页面披露。与 FTX 的差别是 TRX 有全市场盘口而 FTT 没有,HTX 公布了负债表而 FTX 没有;相同点是储备价值是自家系代币价格的函数。

## 5. 官方 PoR 横向(各所页面,快照日各异,见"所"列;第三方托管占储备 >10%、仅自有钱包 ÷ 负债 <100% 标红,与 §10 触发规则一致)

| 所 | 用户负债 | 自报储备 | 第三方托管占储备 | **仅自有钱包 ÷ 负债** |
|---|---|---|---|---|
| Binance(08-01) | $127.9B | $130.3B | 0.9% | 100.9% |
| OKX(21 资产,08 月) | $30.8B | $32.6B | 2.6% | 103.0% |
| HTX(09-01) | $6.03B | $6.37B | <mark class="r">**18.1%**(托管方不披露)</mark> | <mark class="r">**86.5%**</mark> |
| Gate(08-19) | — | 127% | 无托管栏 | — |
| Bitget(08-20) | — | 120%(USDT 100%,零冗余) | 无托管栏 | — |
| KuCoin(08-31) | — | 110% | 无托管栏 | — |
| MEXC(09-09) | — | 141% | 无托管栏 | — |
| Kraken | — | 仅比率 | — | — |
...
| # | 差异项 | 读数 | 怎么核 |
|---|---|---|---|
| 1 | **USDT 自有钱包只覆盖负债 4.8%,93% 的 USDT 储备在第三方托管** | 09-01 页面:负债 872.3M;自有钱包 **41.46M**(÷ 负债 = 4.8%);托管 546.87M(占 USDT 储备 93%)。⚠ 自有钱包的分链构成(08-01:USDT-TRC20 13.24M + USDT-ERC20 1.05M + stUSDT 39.55M,即 73% 是 JustLend 凭证)只在 GitHub CSV 上,09-01 版尚未发布。官方 USDT-TRC20 5 址链上 09-14:**1.91M** | HTX PoR 页 "USDs" 明细;GitHub 快照 CSV;`--chain tron --ex htx`(`tron_por` 项) |
| 2 | **18.1% 储备在托管方未在页面披露的第三方托管** | 2026-06-01 起新设类别;09-01 含 USDT 546.9M、USDC 239.4M、ETH 87,543、BTC 1,689、USDD 6.9M、U 17.9M、SOL 1,833、HTX 币 3,018 亿枚;Binance 同口径 0.9%、OKX 2.6% | HTX PoR 页 "Custodial Wallets" 栏 |
| 3 | **BTC 储备 51% 是 Poloniex 发行的 BTC-TRC20,公布地址与 PoR 页面上未发现对应抵押物(链上检索不能证否)** | 09-01 自报 BTC 20,252 = 交易所钱包 18,563 + 托管 1,689;BTC-TRC20 链上 09-14 为 **10,334 枚 = 自报储备的 51%**;11 个公布地址直读原生 8,203 = 用户负债 19,498 的 **42.1%** | `--chain btc,tron --ex htx`;BTC-TRC20 见 §7.2 |
| 4 | **ETH 储备 75.0% 在托管,链上可核 25.0%** | 09-01 页面:用户 114,984 / HTX 116,757 / 交易所钱包 29,213 / 托管 87,543;PoR 11 址链上 09-14 读到 **$67.4M**(其中原生 ETH 仅 61.6 枚,余为 stETH 等凭证) | `--chain eth --ex htx`(`eth_por` 项) |
| 5 | **TRX 占储备 47%,69% 质押中,而全市场盘口装不下它的 1%** | 18 址持 9.75B TRX = 供给 10.3%(可用 3.01B + 质押中 6.73B,质押占 69%,09-14 直读);09-01 用户 TRX 负债 8.62B ⇒ 即时可付 **35%**;十所现货 ±2% 深度合计 $17.6M,HTX 持仓 $3.2B | trongrid `getaccount`;各所深度接口 |
| 6 | **平台币 HTX 计入储备 6.1%,自报交易所钱包等于其供给的 23.2%,仅本所有成交** | Tron 主链 totalSupply **999.99 万亿枚**(2026-09-07 合约直读);09-01 页面自报交易所钱包 **231.76 万亿枚 = 供给的 23.2%**,另托管 3,018 亿枚 —— 这与上期"自持约 23%"一致,同时确认上期"18 址持 2.31 万亿"是量级笔误(2.31/1000 = 0.23%,差 100 倍),已撤回。⚠ 该 23.2% 为**自报**,持币地址仍不在本报告的 TRX 冷钱包清单内,链上逐址核对未完成;Binance/OKX 未上市,五家挂牌所日成交 $0–5 万;九所无永续 | Tron JSON-RPC;各所行情接口 |
...
| 8 | 页面自洽,但可核部分不到一半;本期还少了一条交叉源 | 09-01 页面 BTC / ETH 储备率 104% / 102%,链上能证明"确实是那个币"的部分分别为 **42.5% / 25.0%**。⚠ **本期无法做"页面 ↔ GitHub 快照逐格核对"**:GitHub 的 `huobi_por.csv` 仍是 08-01 快照(HTX 一贯在快照后约 11 天推送,08-01 版是 08-12 推的),09-01 版预计 09-12 前后;上期(08-01)四个数逐格对上 | 本报告 §3、§5 |
...
| 供给与持有 | 17,545 枚;**[HTX 6](https://tronscan.org/#/address/TDToUxX8sH4z6moQpK3ZLAN24eupu2ivA4) 持 10,299(58.7%)+ [JustLend jBTC 市场](https://tronscan.org/#/contract/TLeEu311Cbw63BcmMHDgDLu7fnk9fqGcqT) 6,563(37.4%)= 96%**;Poloniex 自持 28 |
...
## 8. 关联币可变现性:储备中的持仓 ÷ 30 日均日成交(2026-09-14 实测;关联币占储备 >30%、质押占比 >50% 标红,与 §2/§10 规则一致)

§2 里每家所的"关联币"都按市价计入储备。这一节问同一个问题:**真要卖,市场每天接得住多少?** 读法统一:用储备中该币的持仓除以**近 30 个完整日的日均成交额**(单日成交波动大,30 日均更稳),得到按日均成交量全部出清所需的天数。成交额给两个口径:**主流所** = Binance、OKX、Bybit、KuCoin、Gate、Bitget、Kraken、HTX、MEXC、Bitfinex 中有该交易对者的现货对日 K 线合计(一手;所数见表);**全市场** = CoinGecko 记录的该币全部上市场所(⚠ 第三方,不逐所核)。天数按主流所口径算;全市场只作参照,因为它的大头常来自主流所之外的场所(见读法)。±2% 盘口深度(挂单可即时成交的美元量)列出作参照。脚本 `tools/affiliated_liquidity.py`,读数存 `data/affiliated_liquidity_2026-09-14.json`,持仓按 §2 的 09-14 读数(储备 × 关联币占比);HTX 的 TRX 用本方链上直读、HTX 币沿用上期读数并标 ⚠。

| 所 | 关联币 | 占储备 | 储备中持仓 | 主流所 30 日均日成交(所数) | CoinGecko 全市场 30 日均(⚠) | ±2% 深度(主流所) | 持仓 ÷ 主流所 30 日均成交(全量出清需天数) |
|---|---|---|---|---|---|---|---|
| Binance | BNB | 16% | $27.1B | $168.3M(8 所) | $1,023.3M | $23.6M | ≈161 天 |
| Bitfinex | LEO | <mark class="r">32%</mark> | $6.2B | $0.2M(2 所) | $0.3M | $0.08M | ≈31,000 天 |
| Gate | GT | 15% | $1.07B | $0.9M(1 所) | $2.3M | $0.08M | ≈1,190 天 |
| KuCoin | KCS | 15% | $0.50B | $5.3M(2 所) | $12.8M | $0.03M | ≈94 天 |
| Bitget | BGB | 8% | $0.46B | $9.6M(2 所) | $10.4M | $0.53M | ≈48 天 |
| HTX | TRX | <mark class="r">47.5%</mark>(自报口径) | $3.30B(本方 18 址直读 97.5 亿枚 × $0.338;<mark class="r">69% 质押中</mark>) | $71.7M(10 所;HTX 自家 30 日均 $14.25M) | $405.6M | $16.71M(十所) | ≈46 天 |
| HTX | HTX 币 | 6.1%(自报口径) | $0.39B(09-01 自报 232.06 万亿枚 × $1.68e-6) | $30.2M(6 所;仅 HTX 有成交) | $34.2M | $0.40M(六所) | ≈13 天 |

**读法**

- 出清天数单看会误导,必须与占比并读:Binance 的 BNB 需约 177 天,但 BNB 只占储备 16%,硬资产 72%,不构成问题;Bitfinex 的 LEO 占 33% 且需约 26,700 天(约 73 年,两口径相同),两项同时成立才是可变现性问题。**关联币的"储备价值"是市价乘以数量,不是能变现的钱** —— 这对所有所成立。
- 全市场口径为什么只作参照:CoinGecko 的 TRX 成交里,十家主流所只占 16%,最大的贡献者是 FameEX(12%)、WhiteBIT(7%)、Phemex(6.5%),Binance 仅 7%;BNB 的全市场成交里主流所占 27%,Binance 19%,其余来自 P2B、CoinUp、BTCC、XT 等。两者都说明全市场数字的大头在本报告不逐所核的场所。TRX 按主流所 30 日均出清约 45 天(全市场口径约 8 天);HTX 的可变现性问题不在 TRX 的成交,在**它占储备的比例(47% + 5.7%)与 69% 的质押**。Bitfinex 的 LEO 占 32%,其余 61% 是 BTC;Binance 的 BNB 占 16%,其余 72% 是 BTC、ETH 与稳定币。
- 判断一家所压力下的兑付能力,应把关联币按可成交量而非市价折算后,再看硬资产(BTC / ETH / 稳定币)对负债的覆盖。按此口径(§5 表):Binance、OKX 仍 ≥100%;HTX 的硬资产覆盖不到一半。
...
python3 tools/cex_reserves_verify.py --chain eth --ex okx --retry-failed data/cex_reserves_2026-09-14.json
...
|---|---|---|---|---|---|---|---|---|
| 第三方托管占储备 | 官方 PoR 页托管栏 | >10% | 0.9% | 2.6% | — | — | <mark class="r">18.1%</mark> | — |
| 仅自有钱包 ÷ 用户负债 | 官方 PoR 页 | <100% | 100.9% | 103.0% | ≥100%(USDT 恰 100%) | 127% | <mark class="r">86.5%</mark> | ≥100% |
...
本页为公开数据的整理与核对,不构成投资建议,不构成对任何机构偿付能力的断言。负债端数字全部来自交易所自报,本报告无法验证。本页所有表述均限于可公开核验的数据范围;未在公布地址上发现某项资产,不等于该资产不存在。
