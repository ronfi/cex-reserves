...
|---|---|---|---|---|---|---|---|---|---|
| Binance | 37 + 锁仓 2 | $68.70B | $68.69B | $69.16B | <span class="ok">99%</span> | 0 | 3,267,293 | ? / 3,991,221(08-01) | -18% |
...
| Gate | 11 | 15M | 0M | 63.7M | 58M / 179M(08-19) | <mark class="r">-91.9%</mark>(DefiLlama-Adapters 清单 11 址不是页面全部钱包(Gate 未公布 TRX 地址)) | 660M / 721M(08-19) |
...
|---|---|---|---|
| HTX | <mark class="r">**是**(77% vs 硬资产 20%)</mark> | TRX ≈44 天、HTX 币 ≈14 天 | 公布逐币负债;**19% 储备在托管方未在页面披露,USDT 自有钱包覆盖负债 5.8%**(§6.2) |
| Bitfinex | 否(32% vs 66%) | LEO ≈31,500 天 | 无 PoR 页,负债未公布 |
| KuCoin | 否(16% vs 59%) | KCS ≈94 天 | 仅公布比率 110% |
| SwissBorg | 否(15% vs 63%) | 未测 | 无 PoR 页 |
| Binance | 否(15% vs 74%) | BNB ≈164 天 | 公布逐币负债,仅自有钱包 ÷ 负债 100.9% |
| Gate | 否(14% vs 54%) | GT ≈1,260 天 | 仅公布比率 127% |
...
- 三条同时成立的所,前 20 名里没有;最接近的是 HTX:① 成立,② 出清约需 44 天(TRX 是这些关联币里成交最活跃的),③ 公布了负债,但五分之一储备在托管方未在页面披露。与 FTX 的差别是 TRX 有全市场盘口而 FTT 没有,HTX 公布了负债表而 FTX 没有;相同点是储备价值是自家系代币价格的函数。
...
## 8. 关联币可变现性:储备中的持仓 ÷ 30 日均日成交(2026-09-06 实测;关联币占储备 >30%、质押占比 >50% 标红,与 §2/§10 规则一致)

§2 里每家所的"关联币"都按市价计入储备。这一节问同一个问题:**真要卖,市场每天接得住多少?** 读法统一:用储备中该币的持仓除以**近 30 个完整日的日均成交额**(单日成交波动大,30 日均更稳),得到按日均成交量全部出清所需的天数。成交额给两个口径:**主流所** = Binance、OKX、Bybit、KuCoin、Gate、Bitget、Kraken、HTX、MEXC、Bitfinex 中有该交易对者的现货对日 K 线合计(一手;所数见表);**全市场** = CoinGecko 记录的该币全部上市场所(⚠ 第三方,不逐所核)。天数按主流所口径算;全市场只作参照,因为它的大头常来自主流所之外的场所(见读法)。±2% 盘口深度(挂单可即时成交的美元量)列出作参照。脚本 `tools/affiliated_liquidity.py`,读数存 `data/affiliated_liquidity_2026-09-06.json`,持仓沿用 §2 的 09-04 读数。

| 所 | 关联币 | 占储备 | 储备中持仓 | 主流所 30 日均日成交(所数) | CoinGecko 全市场 30 日均(⚠) | ±2% 深度(主流所) | 持仓 ÷ 主流所 30 日均成交(全量出清需天数) |
|---|---|---|---|---|---|---|---|
| Binance | BNB | 15% | $24.4B | $148.9M(8 所) | $904.8M | $19.4M | ≈164 天 |
| Bitfinex | LEO | <mark class="r">33%</mark> | $6.3B | $0.2M(2 所) | $0.3M | $0.45M | ≈31,500 天 |
| Gate | GT | 14% | $0.88B | $0.7M(1 所) | $1.9M | $0.07M | ≈1,260 天 |
| KuCoin | KCS | 15% | $0.49B | $5.2M(2 所) | $12.8M | $0.06M | ≈94 天 |
| Bitget | BGB | 8% | $0.46B | $8.8M(2 所) | $9.5M | $0.50M | ≈52 天 |
| HTX | TRX | <mark class="r">47%</mark> | $3.2B(<mark class="r">69% 质押中</mark>) | $73.1M(10 所;HTX 自家 30 日均 $2.7M) | $397.3M | $18.3M(十所) | ≈44 天 |
| HTX | HTX 币 | 5.7% | $0.37B | $27.5M(6 所;仅 HTX 有成交) | $31.0M | $0.40M(六所) | ≈14 天 |

**读法**

- 出清天数单看会误导,必须与占比并读:Binance 的 BNB 需约 164 天,但 BNB 只占储备 15%,硬资产 74%,不构成问题;Bitfinex 的 LEO 占 33% 且需约 31,500 天(约 86 年,两口径相同),两项同时成立才是可变现性问题。**关联币的"储备价值"是市价乘以数量,不是能变现的钱** —— 这对所有所成立。
- 全市场口径为什么只作参照:CoinGecko 的 TRX 成交里,十家主流所只占 16%,最大的贡献者是 FameEX(12%)、WhiteBIT(7%)、Phemex(6.5%),Binance 仅 7%;BNB 的全市场成交里主流所占 27%,Binance 19%,其余来自 P2B、CoinUp、BTCC、XT 等。两者都说明全市场数字的大头在本报告不逐所核的场所。TRX 按主流所 30 日均出清约 44 天(全市场口径约 8 天);HTX 的可变现性问题不在 TRX 的成交,在**它占储备的比例(47% + 5.7%)与 69% 的质押**。Bitfinex 的 LEO 占 33%,其余 61% 是 BTC;Binance 的 BNB 占 15%,其余 73% 是 BTC、ETH 与稳定币。
