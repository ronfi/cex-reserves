...
| 所 | ① 关联币是储备主体 | ② 持仓 ÷ 24h 成交(全量出清需天数,§8) | ③ 负债端(§5) |
|---|---|---|---|
| HTX | <mark class="r">**是**(77% vs 硬资产 20%)</mark> | TRX ≈56 天(十所合计)、HTX 币 ≈47 天 | 公布逐币负债;**19% 储备在托管方未在页面披露,USDT 自有钱包覆盖负债 5.8%**(§6.2) |
| Bitfinex | 否(32% vs 66%) | LEO ≈31,500 天 | 无 PoR 页,负债未公布 |
| KuCoin | 否(16% vs 59%) | KCS ≈310 天 | 仅公布比率 110% |
| SwissBorg | 否(15% vs 63%) | 未测 | 无 PoR 页 |
| Binance | 否(15% vs 74%) | BNB ≈190 天 | 公布逐币负债,仅自有钱包 ÷ 负债 100.9% |
| Gate | 否(14% vs 54%) | GT ≈880 天 | 仅公布比率 127% |
...
- ① 只对一家成立。
- ② 单看不区分谁更危险:出清天数从几十天到几万天,要与 ① 并读;关联币占比越高、成交越稀薄,按可成交量折算后的储备缩水越多。
- ③ 公布逐币负债的只有 Binance、OKX、HTX 三家;其余要么只给一个比率,要么没有 PoR 页。
- 三条同时成立的所,前 20 名里没有;最接近的是 HTX:① 成立,② 出清约需 56 天(TRX 是这些关联币里成交最活跃的),③ 公布了负债,但五分之一储备在托管方未在页面披露。与 FTX 的差别是 TRX 有全市场盘口而 FTT 没有,HTX 公布了负债表而 FTX 没有;相同点是储备价值是自家系代币价格的函数。
...
## 8. 关联币可变现性:储备中的持仓 ÷ 24h 成交(2026-09-03 实测;关联币占储备 >30%、质押占比 >50% 标红,与 §2/§10 规则一致)

§2 里每家所的"关联币"都按市价计入储备。这一节问同一个问题:**真要卖,市场每天接得住多少?** 读法统一:用储备中该币的持仓除以全市场 24h 成交额,得到按当日成交量全部出清所需的天数;主场 ±2% 盘口深度(挂单可即时成交的美元量)一并列出作参照。

| 所 | 关联币 | 占储备 | 储备中持仓 | 主场 ±2% 深度 | 24h 成交 | 持仓 ÷ 24h 成交(全量出清需天数) |
|---|---|---|---|---|---|---|
| Binance | BNB | 15% | $24.4B | $10.9M(Binance) | $128M | ≈190 天 |
| Bitfinex | LEO | <mark class="r">33%</mark> | $6.3B | $0.07M(Bitfinex) | $0.2M | ≈31,500 天 |
| Gate | GT | 14% | $0.88B | $0.08M(Gate) | $1.0M | ≈880 天 |
| KuCoin | KCS | 15% | $0.49B | $0.05M(KuCoin) | $1.6M | ≈310 天 |
| Bitget | BGB | 8% | $0.46B | $0.51M(Bitget) | $13.8M | ≈33 天 |
| HTX | TRX | <mark class="r">47%</mark> | $3.2B(<mark class="r">69% 质押中</mark>) | $17.6M(十所合计;HTX 自家 $0.38M) | $57M(十所) | ≈56 天 |
| HTX | HTX 币 | 5.7% | $0.37B | $0.42M(六所合计;仅 HTX 有成交) | $7.8M | ≈47 天 |

**读法**

- 出清天数单看会误导,必须与占比并读:Binance 的 BNB 需 190 天,但 BNB 只占储备 15%,硬资产 74%,不构成问题;Bitfinex 的 LEO 占 33% 且需约 31,500 天(按当前成交额约 86 年),两项同时成立才是可变现性问题。**关联币的"储备价值"是市价乘以数量,不是能变现的钱** —— 这对所有所成立。
- TRX 是这些币里成交最活跃的(十所 24h $57M,永续持仓量 $238M),出清约需 56 天;HTX 的可变现性问题不在 TRX 的成交,在**它占储备的比例(47% + 5.7%)与 69% 的质押**。Bitfinex 的 LEO 占 33%,其余 61% 是 BTC;Binance 的 BNB 占 15%,其余 73% 是 BTC、ETH 与稳定币。
