...
> 各表标题另有各自的读取时间。全部数字可用本仓库 `tools/cex_reserves_verify.py` 重新读取;输出快照在 `data/cex_reserves_2026-09-07.json`。
...
3. **HTX 自报储备率"均超 100%",但按链上可核口径:USDT 自有钱包只覆盖负债的 4.8%,BTC 42%,ETH 25%;总储备的 18.1% 在不披露身份的"第三方托管",47.5% 是 TRX 且其中 69% 处于质押。**(自报数为 2026-09-01 页面快照,链上数为 09-07 直读)
...
## 2. 全景:链上储备前 20 名(DefiLlama 公开地址口径,读取于 2026-09-07 01:08 UTC)
...
§2 是聚合器口径;本节是**本报告自己读链**:拿各所公布的地址(官方 PoR 清单或DefiLlama-Adapters源码,见 `tools/cex_addresses.json`),逐地址读 BTC / ETH 全资产 / Tron,再与聚合器对账。**表由 `tools/make_tables.py` 从 `data/cex_reserves_2026-09-07.json` 生成**,每个地址的读数与失败记录都在该文件里。
...
| Gate | 13/13 | 18,857 | 18,831 | <span class="ok">+0.1%</span> | 22,436 / 27,550(08-19) | <mark class="r">-31.6%</mark>(DefiLlama-Adapters清单 13 址不是页面全部钱包(Gate 未公布 BTC 地址)) |
| Bitget | 19/19 | 29,461 | 29,473 | <span class="ok">-0.0%</span> | 26,811 / 35,837(08-20) | <mark class="r">-17.8%</mark>(适配器 19 址不是页面全部钱包;页面钱包含 BSC/Lightning 等链 535 枚) |
...
| HTX | 11/11 | 8,292 | 8,213 | <span class="ok">+1.0%</span> | 19,498 / 20,252(09-01) | <mark class="r">-59.1%</mark>(自报钱包 20,252 = 交易所钱包 18,563 + 托管 1,689;交易所钱包含 BTC-TRC20,09-07 链上 10,331 枚,扣除后约 8,232,与本表直读 8,292 差 +0.7%) |
...
- HTX 的行是DefiLlama-Adapters清单 11 址;其官方 PoR 清单为 7 址,链上 8,073 枚(§6.2)。
...
|---|---|---|---|---|---|---|---|---|---|
| Binance | 37 + 锁仓 2 | $69.31B | $69.30B | $69.92B | <span class="ok">99%</span> | 0 | 3,244,790 | ? / 3,991,221(08-01) | -19% |
...
| Gate | 11 | 13M | 84M | 80.1M | 58M / 179M(08-19) | <mark class="r">-45.6%</mark>(DefiLlama-Adapters 清单 11 址不是页面全部钱包(Gate 未公布 TRX 地址)) | 660M / 721M(08-19) |
...
- HTX 表中 18 个 Tron 地址是DefiLlama-Adapters清单里的 TRX 冷钱包,上面没有 USDT 属正常;**HTX 官方 PoR 的 USDT-TRC20 另有 5 个地址**:08-01 快照 13.24M,09-07 链上 <mark class="r">1.91M</mark>(低于用户 USDT 负债的 1%,§10 规则;`TK86…` 的 11.33M 已转空);USDT-ERC20 1 址 1.05M → 0。用户 USDT 负债 926M 的其余部分在"ThirdParty"(§6.2)。
- 末两列为各所官方 PoR 自报的 TRX 与 USDT(用户 / 钱包);USDT 是**全链合计**(ERC20 + TRC20 + 其他链),本表 USDT-TRC20 只是其中一条链,故不算差值。HTX 的 TRX 自报钱包 9,384M(09-01)与本表 18 址 9,770M(可用 + 质押,09-07)同量级;其 USDT 自报钱包 588M 中 547M 在"ThirdParty"(93%,§6.2)。
...
| 规模与持有(一手) | BTCTRON 供给 17,545 枚,HTX 持 10,331(2026-09-07 直读),占其 PoR BTC 行 51%;HBTC 供给仅 969.49 枚,HTX 地址持 ≈540(56%) |
...
| # | 差异项 | 读数 | 怎么核 |
|---|---|---|---|
| 1 | **USDT 自有钱包只覆盖负债 4.8%,93% 的 USDT 储备在第三方托管** | 09-01 页面:负债 872.3M;自有钱包 **41.46M**(÷ 负债 = 4.8%);托管 546.87M(占 USDT 储备 93%)。⚠ 自有钱包的分链构成(08-01:USDT-TRC20 13.24M + USDT-ERC20 1.05M + stUSDT 39.55M,即 73% 是 JustLend 凭证)只在 GitHub CSV 上,09-01 版尚未发布。官方 USDT-TRC20 5 址链上 09-07:**1.91M** | HTX PoR 页 "USDs" 明细;GitHub 快照 CSV;`--chain tron --ex htx`(`tron_por` 项) |
...
| 3 | **BTC 储备 51% 是 Poloniex 发行的 BTC-TRC20,公布地址与 PoR 页面上未发现对应抵押物(链上检索不能证否)** | 09-01 自报 BTC 20,252 = 交易所钱包 18,563 + 托管 1,689;BTC-TRC20 链上 09-07 为 **10,331 枚 = 自报储备的 51%**;11 个公布地址直读原生 8,292 = 用户负债 19,498 的 **42.5%** | `--chain btc,tron --ex htx`;BTC-TRC20 见 §7.2 |
| 4 | **ETH 储备 75.0% 在托管,链上可核 25.0%** | 09-01 页面:用户 114,984 / HTX 116,757 / 交易所钱包 29,213 / 托管 87,543;PoR 11 址链上 09-07 读到 **$68.5M**(其中原生 ETH 仅 84.7 枚,余为 stETH 等凭证) | `--chain eth --ex htx`(`eth_por` 项) |
| 5 | **TRX 占储备 47%,69% 质押中,而全市场盘口装不下它的 1%** | 18 址持 9.77B TRX = 供给 10.3%(可用 3.04B + 质押中 6.73B,质押占 69%,09-07 直读);09-01 用户 TRX 负债 8.62B ⇒ 即时可付 **35%**;十所现货 ±2% 深度合计 $17.6M,HTX 持仓 $3.2B | trongrid `getaccount`;各所深度接口 |
...
## 8. 关联币可变现性:储备中的持仓 ÷ 30 日均日成交(2026-09-07 实测;关联币占储备 >30%、质押占比 >50% 标红,与 §2/§10 规则一致)

§2 里每家所的"关联币"都按市价计入储备。这一节问同一个问题:**真要卖,市场每天接得住多少?** 读法统一:用储备中该币的持仓除以**近 30 个完整日的日均成交额**(单日成交波动大,30 日均更稳),得到按日均成交量全部出清所需的天数。成交额给两个口径:**主流所** = Binance、OKX、Bybit、KuCoin、Gate、Bitget、Kraken、HTX、MEXC、Bitfinex 中有该交易对者的现货对日 K 线合计(一手;所数见表);**全市场** = CoinGecko 记录的该币全部上市场所(⚠ 第三方,不逐所核)。天数按主流所口径算;全市场只作参照,因为它的大头常来自主流所之外的场所(见读法)。±2% 盘口深度(挂单可即时成交的美元量)列出作参照。脚本 `tools/affiliated_liquidity.py`,读数存 `data/affiliated_liquidity_2026-09-07.json`,持仓按 §2 的 09-07 读数(储备 × 关联币占比);HTX 的 TRX 用本方链上直读、HTX 币沿用上期读数并标 ⚠。

| 所 | 关联币 | 占储备 | 储备中持仓 | 主流所 30 日均日成交(所数) | CoinGecko 全市场 30 日均(⚠) | ±2% 深度(主流所) | 持仓 ÷ 主流所 30 日均成交(全量出清需天数) |
|---|---|---|---|---|---|---|---|
| Binance | BNB | 16% | $27.1B | $152.6M(8 所) | $927.0M | $21.1M | ≈177 天 |
| Bitfinex | LEO | <mark class="r">32%</mark> | $6.2B | $0.2M(2 所) | $0.3M | $0.45M | ≈26,700 天 |
| Gate | GT | 15% | $1.07B | $0.7M(1 所) | $1.9M | $0.10M | ≈1,430 天 |
| KuCoin | KCS | 15% | $0.50B | $5.2M(2 所) | $12.8M | $0.05M | ≈96 天 |
| Bitget | BGB | 8% | $0.46B | $8.8M(2 所) | $9.5M | $0.45M | ≈52 天 |
| HTX | TRX | <mark class="r">47.5%</mark>(自报口径) | $3.27B(本方 18 址直读 97.7 亿枚 × $0.335;<mark class="r">69% 质押中</mark>) | $72.4M(10 所;HTX 自家 30 日均 $2.6M) | $394.9M | $16.9M(十所) | ≈45 天 |
| HTX | HTX 币 | 6.1%(自报口径) | $0.39B(09-01 自报 232.06 万亿枚 × $1.68e-6) | $28.2M(6 所;仅 HTX 有成交) | $31.5M | $0.42M(六所) | ≈14 天 |
