...
def _day(s):
    if not s: return '?'
    m = re.search(r'(\d{4})-(\d{2})-(\d{2})', str(s)); return f'{m.group(2)}-{m.group(3)}' if m else str(s)[:8]
def por(ex, coin):
...
for ex in ORDER:
    ...
    out.append(f"| {NAME[ex]} | {n} | ${full/1e9:.2f}B | ${cal/1e9:.2f}B | ${ll/1e9:.2f}B | {covs} | {len(e['failed'])} | {por_cell(ex, 'ETH')} |")
out.append('\n@@TRON@@\n' + S['h33'])
out.append(S['t33'])
out.append('|---|---|---|---|---|---|---|')
for ex in ORDER:
    t = r[ex].get('tron')
    if not t: continue
    out.append(f"| {NAME[ex]} | {t['n']} | {t['trx_available']/1e6:,.0f}M | {t['trx_frozen']/1e6:,.0f}M | {t['usdt_trc20']/1e6:,.1f}M | {por_cell(ex, 'TRX', 1e6, '{:,.0f}M')} | {por_cell(ex, 'USDT', 1e6, '{:,.0f}M')} |")
print('\n'.join(out))
