...
LANG = {
    'zh': dict(
        ...
        legend=[('<span class="ok">绿</span>', '与聚合器对上(规则见各表标题)'), ('<mark class="r">红</mark>', '按同一规则判定的异常,不分交易所'), ('<mark class="n">黄 *</mark>', '口径差,表下有注')],
    ...
    'en': dict(
        ...
        legend=[('<span class="ok">green</span>', 'reconciles with the aggregator (rule in each table heading)'), ('<mark class="r">red</mark>', 'anomaly by the same rule, regardless of exchange'), ('<mark class="n">yellow *</mark>', 'caliber difference, note under the table')],
